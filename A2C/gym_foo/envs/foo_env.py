...
class FooEnv(gym.Env):
    ...
    def __init__(self, LoadData=True, Train=True, multiUT=False, Trajectory_mode='Kmeans', MaxStep=41):
        globe._init()
        ...
        # the path loss at the reference distance D0 = 1m, 3dB;
        ...
        if LoadData == True:
            if Train == True:
                if multiUT == True:
                    UT_0 = np.loadtxt(
                        "../CreateData/Train_Trajectory_UT_0.csv", delimiter=",")
                    UT_1 = np.loadtxt(
                        "../CreateData/Train_Trajectory_UT_1.csv", delimiter=",")
                    UT_2 = np.loadtxt(
                        "../CreateData/Train_Trajectory_UT_2.csv", delimiter=",")
                    ...
                    if Trajectory_mode == 'Fermat':
                        UAV_Trajectory = np.loadtxt(
                            "../CreateData/Fermat_Train_Trajectory_3.csv", delimiter=",")
                    else:
                        UAV_Trajectory = np.loadtxt(
                            "../CreateData/Kmeans_Train_Trajectory_3.csv", delimiter=",")
                    ...
                    if Trajectory_mode == 'Fermat':
                        UAV_Trajectory = np.loadtxt(
                            "../CreateData/Fermat_Train_Trajectory_1.csv", delimiter=",")
                    else:
                        UAV_Trajectory = np.loadtxt(
                            "../CreateData/Kmeans_Train_Trajectory_1.csv", delimiter=",")
                ...
                if multiUT == True:
                    UT_0 = np.loadtxt(
                        "../CreateData/Test_Trajectory_UT_0.csv", delimiter=",")
                    UT_1 = np.loadtxt(
                        "../CreateData/Test_Trajectory_UT_1.csv", delimiter=",")
                    UT_2 = np.loadtxt(
                        "../CreateData/Test_Trajectory_UT_2.csv", delimiter=",")
                    ...
                    if Trajectory_mode == 'Fermat':
                        UAV_Trajectory = np.loadtxt(
                            "../CreateData/Fermat_Test_Trajectory_3.csv", delimiter=",")
                    else:
                        UAV_Trajectory = np.loadtxt(
                            "../CreateData/Kmeans_Test_Trajectory_3.csv", delimiter=",")
                    ...
                    if Trajectory_mode == 'Fermat':
                        UAV_Trajectory = np.loadtxt(
                            "../CreateData/Fermat_Test_Trajectory_1.csv", delimiter=",")
                    else:
                        UAV_Trajectory = np.loadtxt(
                            "../CreateData/Kmeans_Test_Trajectory_1.csv", delimiter=",")

                    globe.set_value('UAV_Trajectory', UAV_Trajectory)

        self.action_space = spaces.Box(0, 1, shape=(34, ), dtype=np.float32)
        self.observation_space = spaces.Box(
            0, 20, shape=(2, ), dtype=np.float32)
    ...
    def step(self, action):
        ...
        reward, radio_state, received_energy = self.env_state(
            step, tau, power_1, Theta_R, Omega_R)
        done = False
        if step == t - 1:
            done = True
    ...
    def reset(self):
        ...
        distance_AP_RIS = mt.sqrt(mt.pow(
            (L_U[0] - L_AP[0]), 2) + mt.pow((L_U[1] - L_AP[1]), 2) + mt.pow((L_U[2] - L_AP[2]), 2))
        distance_RIS_UT_0 = mt.sqrt(mt.pow(
            (L_U[0] - UT_0[0]), 2) + mt.pow((L_U[1] - UT_0[1]), 2) + mt.pow((L_U[2] - UT_0[2]), 2))
    ...
    def pl_BR(self, L_U, L_AP):
        ...
        theta = (180 / mt.pi) * mt.asin(((L_U[2] - L_AP[2]) / mt.sqrt(
            mt.pow(L_U[0], 2) + mt.pow(L_U[1], 2) + mt.pow((L_U[2] - L_AP[2]), 2))))
        p_los = 1 + a * mt.exp(a * b - b * theta)
        p_los = 1 / p_los

        p_nlos = 1 - p_los
        # channel power gain (BS-RIS) with the los and nlos
        g_BR = (p_los + p_nlos * varphi) * mt.pow(mt.sqrt(mt.pow(L_U[0], 2) + mt.pow(
            L_U[1], 2) + mt.pow((L_U[2] - L_AP[2]), 2)), (0-alpha))

        return g_BR

    def SmallFading_G(self, BS_Z, RIS_L):
        SmallFading_G = 1/np.sqrt(2)*(np.random.normal(loc=0, scale=1, size=(
            BS_Z, RIS_L)) + 1j*np.random.normal(loc=0, scale=1, size=(BS_Z, RIS_L)))
    ...
    def EH(self, tau, power_1, Theta_R, L_U, L_AP, Omega_R):
        ...
        for i in range(0, BS_Z):
            for j in range(0, RIS_L):
                g_ij = [x[i][j]]
                g_hat_2norm[i][j] = np.linalg.norm(
                    g_ij, ord=2, keepdims=True) * (1 - Omega_R[j])
    ...
    def Rayleigh_RU(self, L_U, UT, BS_Z, RIS_L):
        Rayleigh_RU = 1/np.sqrt(2)*(np.random.normal(loc=0, scale=1, size=(
            RIS_L, 1)) + 1j*np.random.normal(loc=0, scale=1, size=(RIS_L, 1)))
        return Rayleigh_RU
    ...
    def Channel_RU(self, L_U, UT, BS_Z, RIS_L):
        ...
        distance = mt.sqrt(mt.pow(
            (L_U[0] - UT[0]), 2) + mt.pow((L_U[1] - UT[1]), 2) + mt.pow((L_U[2] - UT[2]), 2))
        PL = np.sqrt(kappa * mt.pow((distance/1), -hat_alpha))

        h_ru = h_ru * np.sqrt(5/(1+5)) * PL + np.sqrt(1/(1+5)) * \
            PL * self.Rayleigh_RU(L_U, UT, BS_Z, RIS_L)
    ...
    def capacity(self, tau, power_1, Theta_R, L_U, L_AP, UT_0, Omega_R):
        ...
        signal_UT_1 = np.sum(
            np.abs(UT_link_1 * np.conjugate(UT_link_1))) * power_1
    ...
    def env_state(self, step, tau, power_1, Theta_R, Omega_R):
        ...
        reward, received_energy = self.EH(
            tau, power_1, Theta_R, L_U, L_AP, Omega_R)
        HarvestEnergy = reward
        Aver_Throughput = self.capacity(
            tau, power_1, Theta_R, L_U, L_AP, UT_0, Omega_R)
